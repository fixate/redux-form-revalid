...
export default function isEmail(name, options = defaultOptions) {
  return (value = '') => {
    ...
    if (!isByteLength(user, { max: 64 }) ||
      ...
      return { type: 'email', reason: 'invalidLength', value, name };
    }

    if (!isFQDN(domain, { require_tld: opts.require_tld })) {
      return { type: 'email', reason: 'invalidFQDN', value, name };
    }
    ...
    if (user[0] === '"') {
      ...
      if (opts.allow_utf8_local_part) {
        if (!quotedEmailUserUtf8.test(user)) {
          return { type: 'email', reason: 'invalidUser', value, name };
    ...
    for (let i = 0; i < userParts.length; i++) {
      if (!pattern.test(userParts[i])) {
        return { type: 'email', reason: 'invalidPattern', value, name };
      }
    }
